...
class MainWindow(QMainWindow):
    ...
    def compute(self, compute_type):
        ...
        if compute_type == 0:
            self.ca.refresh_data_1(input_value)
            result = self.ca.sin_func()  # 计算sin
            if len(str(result)) > 18:  # 长度超过显示长度，截取字符串
                result = str(result)[:18-len(str(result))]
        elif compute_type == 1:
            self.ca.refresh_data_1(input_value)
            result = self.ca.cos_func()  # 计算cos
            if len(str(result)) > 18:  # 长度超过显示长度，截取字符串
                result = str(result)[:18-len(str(result))]
        elif compute_type == 2:
            result = "计算过程未实现"  # 计算arctan
        else:
            result = "计算过程未实现"  # 计算arcsin
